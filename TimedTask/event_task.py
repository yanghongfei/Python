...
from TimedTask.settings import EMAIL_INFO
from TimedTask.utils.logger import log

# 全局ORM对象
event_orm = orm.EventReminderORM()
#定义一个log

logger = log(log_name='event_reminder')
...
def send_mail(content, email):
    ...
    subject = "%s: AiOps平台事件提醒" % (time.ctime())
...
def check_reminder_event():
    ...
    :return:
    """
    for event in event_orm.get_all_event():
        ...
        if start_reminder_time <= now_time:
            logger.info('EventReminder Name:{}, EventReminder Content:{}, EventReminder Email：{}, EventReminder Time: {}'.format(event.name, event.content, event.email, now_time))
            # print('EventReminder Has: {}'.format(event.name))
            content = """
                    <!DOCTYPE html><html>
                    <head lang="en">
                    <meta charset="UTF-8">
...
def exec_task():
    ...
    sched.add_job(check_reminder_event, 'cron', day_of_week='0-6', hour=9, minute=00, second=00)    #每天早上9点
    sched.start()
